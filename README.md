...
运行 **`dist/MapleFarmUnified-v2.2.3.exe`**，在顶部选择模式：随机跳攻（野猪领地1）、绳边射手（猴子沼泽3），自定义守台，或仅定时 Buff。四个设置页共用启动、暂停、停止和日志；切换设置页不会切换正在运行的模式，Buff 开关可实时应用，槽位修改点“应用 Buff 设置”；其他参数需停止后重新启动。带版本号的新版与原 `MapleFarmUnified.exe` 共用配置，切换前请关闭旧程序。

自定义守台在 v2.2.1 简化为 **三步标定**：① 固定参照物；② 人物名字；③ 怪物观察区域与怪物框。框完自动进入下一步，第三步可连续添加多个怪物。截图时的人物位置自动成为回位目标，无需再框平台或停靠范围；支持右侧、左侧、两侧攻击，旧方案仍可载入。操作说明见 [自定义守台说明](templates/platform_guard_example/README.md)。

v2.2.3 修复观察区及射程边缘的怪物模板被裁切而漏识别的问题：按模板中心判断是否在区域内。人物名字优先完整匹配，局部遮挡时要求至少三段唯一匹配且推导同一位置，不沿用上一次坐标。匹配失败日志区分低分与多个候选，每五秒最多保存一次原图及加载模板信息到 `captures/guard_diagnostics/`。

v2.2.2 优化守台识别：优先核对标定时参照物的位置，多张怪物模板并行匹配，保留原有彩色匹配阈值与 120ms 过期帧保护。超时日志分别显示截图、识别耗时；窗口 / 暂停状态变化单独提示，恢复后显示本帧耗时。
