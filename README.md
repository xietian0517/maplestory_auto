...
新增独立方案 **`rope_archer`（绳边射手 v1.6.0）**：可以复制截图，在界面「粘贴图片」或图片区域 **Ctrl+V**，框选名字并保存为自己的模板；也支持输入角色名进行本地 OCR 定位。右侧有猴子时持续按住 Shift，无怪松开；支持光圈辅助识别，击退后持续按住方向键回到平台内侧，移动按剩余距离提前松键。
...
仓库包含 v1.6.0 的 Windows 发行包。克隆仓库或下载并解压整个仓库后，直接打开
